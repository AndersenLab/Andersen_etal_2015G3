...
# N2xCB4856.cross$pheno$id <- as.integer(as.numeric(as.character(sapply(as.character(N2xCB4856.cross$pheno$id), function(x){strsplit(x, "X")[[1]][2]}))))

# Reorder by trait id
trait3 <- trait3[order(trait3$id),]
trait3 <- trait3[trait3$id>239,]

N2xCB4856.cross$pheno <- trait3
...
impcross = N2xCB4856.cross

# pdata.01 = extractScaledPhenotype(impcross)

# Use this one
pdata.01s = extractScaledPhenotype(impcross, set=impcross$pheno$set, setCorrect=F)
...
marker.chr.cnt=sapply(impcross$geno, function(x) ncol(x$data))
